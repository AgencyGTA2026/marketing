...
export async function notifyOwner(subject: string, message: string, scheduledFor?: Date) {
  ...
  const result = await resendClient().emails.send({
    ...
    to,
    subject,
    html: `<div style="font-family:Arial,sans-serif;color:#121820;max-width:600px;margin:auto;padding:36px"><p style="color:#2457e6;font-size:11px;letter-spacing:.14em">BAYLINE DIGITAL / SOCIAL STUDIO</p><h1 style="font-size:28px">${subject}</h1><p>${message}</p>${detail}<p><a href="${process.env.BETTER_AUTH_URL}/studio/social">Open Social Studio</a></p></div>`,
    text: `${subject}\n\n${message}${scheduledFor ? `\nScheduled: ${formatStudioDate(scheduledFor)}` : ""}\n\n${process.env.BETTER_AUTH_URL}/studio/social`,
  });
