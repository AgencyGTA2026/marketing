...
export function LocationPage({ location }: { location: LocationData }) {
  ...
  return <><LocationPageView city={location.cityName} /><Nav /><main className="location-page">
    <Breadcrumbs items={[{ label: "Locations", href: "/locations" }, { label: location.cityName, href: `/locations/${location.citySlug}` }]} />
    ...
    <section className="location-closing wrap"><p className="eyebrow">YOUR CURRENT HOMEPAGE IS ENOUGH TO START</p><h2>We will show you the first thing worth fixing.</h2><a className="text-link" href="#contact">Request a free homepage review <span aria-hidden="true">↗</span></a></section>
  </main><Footer /></>;
}
