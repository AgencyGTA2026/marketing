...
export function MobileStickyCTA({ city }: MobileStickyCTAProps) {
  ...
  return (
    <div className="fixed inset-x-0 bottom-0 z-50 flex gap-2.5 border-t border-line bg-bg-card/95 px-4 py-3 backdrop-blur-sm md:hidden">
      <a
        href="#contact"
        onClick={() => trackClientEvent("book_call_click", { city, page_type: "location", location: "mobile_sticky" })}
        className="flex flex-1 items-center justify-center rounded-full bg-blue px-4 py-3 text-[15px] font-medium text-white"
      >
        Book Free Audit
      </a>
      <a
        ...
        className="flex items-center justify-center rounded-full border border-line-2 bg-bg px-5 py-3 text-[15px] font-medium text-ink"
      >
        Call
      </a>
    </div>
